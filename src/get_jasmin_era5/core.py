...
class Find_era5:
    ...
    def __getitem__(self, args):
        ...
        if isinstance(var, str):
            var = [var]
        files = sum([self.find_files(v, dates, model=model) for v in var if v not in self._INVARIANTS], [])
        if len(files) > 0:
            ds = xr.open_mfdataset(files, combine="by_coords")
            ds = ds.sel({k:v for k,v in sel.items() if k in ds.coords})
        invar_files = sum([self.find_files(v, dates, model=model) for v in var if v in self._INVARIANTS], [])
    ...
    def find_files(self, var: str, dates: list[datetime], model: str = "oper") -> list[str]:
        if var in self._INVARIANTS:
            ...
            return sum([self.find_single_file(var, date, model=model) for date in dates], [])
...
class Ensemble_era5(Find_era5):
    ...
    def __getitem__(self, args):
        ...
        if isinstance(var, str):
            var = [var]
        files = [self.find_files(v, dates, model="enda") for v in var if v not in self._INVARIANTS]
        if len(files) > 0:
            ds_list = [[xr.open_mfdataset(f, combine="nested", concat_dim=("ensemble_member")).assign_coords({"ensemble_member": range(1, len(f)+1)}) for f in fl if len(fl)] for fl in files]
            ds = xr.merge([xr.concat(dsl, dim="time") for dsl in ds_list if len(dsl)])
            ds = ds.sel({k:v for k,v in sel.items() if k in ds.coords})
        invar_files = sum([self.find_files(v, dates, model="oper") for v in var if v in self._INVARIANTS], [])
        if len(invar_files) > 0:
            invar_ds = xr.open_mfdataset(invar_files, combine="by_coords").squeeze(drop=True)
            invar_ds = invar_ds.sel({k:v for k,v in sel.items() if k in invar_ds.coords})
            if len(files) > 0:
    ...
    def find_files(self, var: str, dates: list[datetime], model: str = "enda") -> list[str]:
        if var in self._INVARIANTS:
            return self.find_invariant(var)
        else:
    ...
    def find_single_file(self, var: str, date: datetime, model: str = "enda") -> list[str]:
        if model == "enda":
            level_type = "an_sfc"
        else:
            level_type = "*"
